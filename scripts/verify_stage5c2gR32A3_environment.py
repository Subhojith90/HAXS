...
from stage5c2gR32A3_common import load_candidate, sha256_file, sha256_payload, strict_json, verify_record
...
def verify_environment(root: Path, candidate: dict | None = None, install_wheel: bool = True) -> dict:
    if not sys.flags.isolated or sys.flags.no_user_site != 1:
        raise RuntimeError("R3.2A.3 environment verification requires -I")
    ...
    if declared.get("schema_version") != "haxs.stage5c2gR32A3.environment.v1" or declared.get("environment_sha256") != sha256_payload(canonical):
        raise RuntimeError("R3.2A.3 environment self-identity failed")
    ...
    if install_wheel:
        with tempfile.TemporaryDirectory(prefix="haxs-stage5c2gR32A3-install-") as directory:
    ...
    return {
        "stage": "stage5c2gR32A3_environment", "status": "PASS",
