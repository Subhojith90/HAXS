...
from stage5c2gR32A4_semantics import verify_command_record, verify_junit_semantics
...
def verify_host_record(
    ...
    if set(host) != required:
        raise RuntimeError(f"{label} host record keys differ from the R3.2A.4 schema")
    if (
        host["schema_version"] != "haxs.stage5c2gR32A4.physical-host-g0.v1"
        or host["status"] != "PASS"
    ...
    if (
        ledger.get("schema_version") != "haxs.stage5c2gR32A4.named-tests.v1"
...
def recompute_two_host_g0(
    ...
    return {
        "schema_version": "haxs.stage5c2gR32A4.two-host-g0.v1",
        "status": "PASS",
