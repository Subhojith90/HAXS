...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--results',default='results/stage5a_lite'); ap.add_argument('--out',default='results/stage5a_lite/decision'); args=ap.parse_args()
    ...
    pd.DataFrame(table).to_csv(out/'stage5a_decision_table.csv',index=False)
    (out/'stage5a_decision.json').write_text(json.dumps({'stage':'stage5a_convergence_replication_gate','route':route,'passed':passed,'publication_claim_allowed':False,'stage5b_design_review_allowed':bool(passed),'note':'Stage 5A is a convergence/replication gate, not a manuscript stage.'},indent=2))
    print(f'stage5a decision wrote {out}; route={route}')
