...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--results',default='results/stage4e_lite'); ap.add_argument('--out',default='results/stage4e_lite/decision')
    args=ap.parse_args(); res=ROOT/args.results; out=ensure_dir(ROOT/args.out)
    man=json.loads((res/'trajectory_stabilization/stage4e_manifest.json').read_text())
    ...
    decision={
        'stage':'stage4e_3x3x2_trajectory_stabilization',
        'route':route,
        ...
        'stage5_design_review_ready': route in ['stage4e_trajectory_stabilized_prepare_stage5_publication_design','stage4e_signal_survives_but_needs_more_trajectory_or_seed_power'],
    ...
    save_json(out/'stage4e_decision.json',decision); save_dataframe(out/'stage4e_decision_table.csv',tab,{})
    print(f'stage4e decision wrote {out}; route={route}')
