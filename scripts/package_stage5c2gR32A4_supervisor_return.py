...
from stage5c2gR32A4_common import load_candidate, sha256_file, sha256_payload
from stage5c2gR32A4_g0 import finalize_comparison, recompute_two_host_g0

PREFIX = "HAXS_Stage5C2G_R3_2A_4_Complete_G0_Return"
...
def main() -> None:
    ...
    if not args.protocol.is_file() or args.protocol.is_symlink():
        raise RuntimeError("protocol archive is missing or symlinked")
    ...
    with tempfile.TemporaryDirectory(prefix="haxs-stage5c2gR32A4-return-") as directory:
        return_root = Path(directory) / PREFIX
        evidence = return_root / "evidence"
        copy_clean(args.host_a_root.absolute(), evidence / "HOST_A")
        copy_clean(args.host_b_root.absolute(), evidence / "HOST_B")
        host_a = evidence / "HOST_A/HOST_A.json"
        host_b = evidence / "HOST_B/HOST_B.json"
        comparison = finalize_comparison(recompute_two_host_g0(
            host_a, host_b, return_root, candidate, protocol_sha,
        ))
        ...
        record = {
            ...
            "G1_authorized": False,
            "synthetic_dry_run": bool(args.synthetic_dry_run),
            "return_sha256": "",
        }
    ...
    print(json.dumps({
        "stage": "stage5c2gR32A4_complete_g0_return", "status": "PASS",
        ...
        "synthetic_dry_run": bool(args.synthetic_dry_run),
        "next": (
            "LOCAL_ACCEPTANCE_ONLY_NEVER_RECEIPT_ELIGIBLE"
            if args.synthetic_dry_run else "SUPERVISORY_REVIEW_NO_RECEIPT_NO_G1"
        ),
