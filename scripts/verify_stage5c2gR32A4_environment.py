...
from stage5c2gR32A4_common import load_candidate, sha256_file, sha256_payload, strict_json, verify_record
from verify_stage5c2gR32A2_environment import (
    DANGEROUS_ENVIRONMENT, current_environment, installed_wheel_tree_identity,
)
...
def verify_environment(root: Path, candidate: dict | None = None, install_wheel: bool = True) -> dict:
    if not sys.flags.isolated or sys.flags.no_user_site != 1:
        raise RuntimeError("R3.2A.4 environment verification requires -I")
    ...
    if declared.get("schema_version") != "haxs.stage5c2gR32A4.environment.v1" or declared.get("environment_sha256") != sha256_payload(canonical):
        raise RuntimeError("R3.2A.4 environment self-identity failed")
    ...
    wheelhouse = verify_record(root, declared["wheelhouse_manifest"], "wheelhouse manifest")
    if declared["wheelhouse_manifest"] != candidate["wheelhouse_manifest"]:
        raise RuntimeError("candidate and environment wheelhouse identities differ")
    ...
    if install_wheel:
        with tempfile.TemporaryDirectory(prefix="haxs-stage5c2gR32A4-install-") as directory:
            ...
            if completed.returncode:
                raise RuntimeError("candidate wheel installation failed")
            identity = installed_wheel_tree_identity(target, wheel)
            installed_sha = identity["payload_tree_sha256"]
            if installed_sha != declared["installed_wheel_tree_sha256"]:
                raise RuntimeError("installed wheel tree identity failed")
    return {
        "stage": "stage5c2gR32A4_environment", "status": "PASS",
