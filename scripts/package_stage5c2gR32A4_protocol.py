...
from stage5c2gR32A4_common import safe_relative, sha256_file
...
def main() -> None:
    output = ROOT / "output/stage5c2gR32A4"
    output.mkdir(parents=True, exist_ok=True)
    required = [
        ROOT / "results/stage5c2gR32A4/protocol/CANDIDATE.json",
        ROOT / "results/stage5c2gR32A4/protocol/NAMED_TEST_LEDGER.json",
        ROOT / "results/stage5c2gR32A4/protocol/ROOT_MANIFEST.json",
        ROOT / "results/stage5c2gR32A4/environment.json",
        ROOT / "results/stage5c2gR32A4/adversarial/OUTCOMES.json",
        ROOT / "output/stage5c2gR32A4/haxs-0.8.7-py3-none-any.whl",
    ]
    ...
    if archive.exists() or sidecar.exists():
        raise RuntimeError("refusing to overwrite R3.2A.4 protocol")
    ...
    print(json.dumps({"stage": "stage5c2gR32A4_protocol", "status": "PASS", "archive": str(archive), "sha256": digest, "content_files": len(unique), "G1_authorized": False, "scientific_execution_performed": False}, indent=2, sort_keys=True))
