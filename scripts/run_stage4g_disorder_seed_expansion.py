...
def main():
    ...
    ap.add_argument('--config', default='configs/stage4g_lite/disorder_seed_expansion.yaml')
    ap.add_argument('--out', default='results/stage4g_lite/disorder_seed_expansion')
    args=ap.parse_args()
    raw=yaml.safe_load((ROOT/args.config).read_text())
    out=ensure_dir(ROOT/args.out)
    gen=ensure_dir(out/'_generated_configs')
    st4e=raw.get('stage4g',{})
    ...
    if passed:
        route='stage4g_disorder_seed_passed_prepare_stage5_design_review'
    elif neg_ok and (t_ok or bool(final.get('bootstrap_ci_excludes_zero',False))):
        route='stage4g_signal_survives_needs_stage5_power'
    elif neg_ok:
        route='stage4g_directional_only_not_ready'
    else:
        route='stage4g_signal_not_stable_reconsider_route'
    ...
    save_dataframe(out/'stage4g_trajectory_scaling_summary.csv',sdf,raw)
    save_dataframe(out/'stage4g_shape_readiness.csv',pd.DataFrame(shape_rows),raw)
    save_dataframe(out/'stage4g_power_projection.csv',pd.DataFrame(power_rows),raw)
    save_dataframe(out/'stage4g_readiness_gates.csv',readiness,raw)
    save_dataframe(out/'stage4g_recommended_stage5_design.csv',stage5,raw)
    save_json(out/'stage4g_disorder_seed_manifest.json',{'stage':'stage4g_disorder_seed_expansion_pilot','config':args.config,'target_shape':target_shape,'ntraj_sweep':ntraj_sweep,'trajectory_reps':reps,'disorder_seeds':seeds,'route':route,'passed':passed,'final_summary':final,'claim_scope':'trajectory-stabilization pilot only; no final publication claim'})
    print(f'stage4g disorder-seed expansion wrote {out}; route={route}; passed={passed}; final_ntraj={final.get("n_traj",None)}; traj_fraction={final.get("trajectory_fraction",None)}')
