...
def main():
    ...
    ap.add_argument('--config',default='configs/stage5b0R_lite/adaptive_trajectory_lock_and_five_label_3x3x2.yaml')
    ap.add_argument('--out',default='results/stage5b0R_lite')
    args=ap.parse_args()
    parent=ROOT/args.config; raw=yaml.safe_load(parent.read_text()); parent_hash=sha(parent)
    out=ensure_dir(ROOT/args.out); lock=ensure_dir(out/'adaptive_trajectory_fraction_lock'); gen=ensure_dir(lock/'_generated_configs')
    st=raw.get('stage5b0R',{})
    target=str(st.get('target_shape','3x3x2'))
    ...
    seeds=int(st.get('seeds_per_block',8))
    transcript=[]; summary=[]; seed_rows=[]; selected=None
    candidates=st.get('adaptive_candidates',[{'ntraj':128,'trajectory_reps':12},{'ntraj':192,'trajectory_reps':8},{'ntraj':256,'trajectory_reps':8}])
    blocks=[('primary',int(st.get('primary_seed_start',91001))),('replication',int(st.get('replication_seed_start',99001)))]
    for cand in candidates:
        ntraj=int(cand.get('ntraj',128)); reps=int(cand.get('trajectory_reps',8)); cand_rows=[]; cand_seeds=[]
        for block, seed_start in blocks:
            ...
            cfg['stage4']['matched_families']=[{'family':st.get('target_family','target_3d_adaptive_trajectory_lock'),'shapes':[[3,3,2]]}]
            cfg_path=gen/f'stage5b0R_{block}_lock_ntraj_{ntraj}_reps_{reps}.yaml'; cfg_path.write_text(yaml.safe_dump(cfg,sort_keys=False))
            camp=lock/f'{block}_campaign_ntraj_{ntraj}_reps_{reps}'; diag=lock/f'{block}_diagnosis_ntraj_{ntraj}_reps_{reps}'
            ...
            row,sr=summarize_campaign(camp,cfg_path,parent_hash,block,ntraj,target,a,b,ci)
            row['trajectory_reps_lock']=reps
            cand_rows.append(row); cand_seeds.extend(sr)
        sdf_c=pd.DataFrame(cand_rows)
        sdf_c['stage5b0R_block_passed']=[block_ok(r,neg_thr,traj_thr) for r in sdf_c.to_dict('records')]
        sdf_c['failure_reasons']=[failure_reasons(r,neg_thr,traj_thr) for r in sdf_c.to_dict('records')]
        prim=sdf_c[sdf_c.block=='primary'].iloc[0].to_dict(); repl=sdf_c[sdf_c.block=='replication'].iloc[0].to_dict()
        block_delta=abs(float(prim['mean_fixed_effect_db'])-float(repl['mean_fixed_effect_db']))
        compatible=bool(block_delta<=compat_thr)
        lock_passed=bool(sdf_c['stage5b0R_block_passed'].all() and compatible)
        sdf_c['block_effect_abs_delta_db']=block_delta
        sdf_c['block_effects_compatible']=compatible
        sdf_c['candidate_lock_passed']=lock_passed
        summary.extend(sdf_c.to_dict('records')); seed_rows.extend(cand_seeds)
        if lock_passed and selected is None:
            selected={'ntraj':ntraj,'trajectory_reps':reps,'block_delta':block_delta}
            break
    sdf=pd.DataFrame(summary); seed_df=pd.DataFrame(seed_rows)
    lock_passed=selected is not None
    five_dir=ensure_dir(out/'five_label_mechanism')
    five_ran=False; five_passed=False; mech=pd.DataFrame()
    if lock_passed or not bool(st.get('run_five_label_only_if_lock_passes',True)):
        ...
        ntraj=int(st.get('five_label_ntraj', selected['ntraj'] if selected else candidates[-1]['ntraj']))
        reps=int(st.get('five_label_trajectory_reps',6))
        cfg=dict(raw); cfg['dtwa']=dict(raw.get('dtwa',{})); cfg['dtwa']['n_traj']=ntraj
        cfg['stage4']=dict(raw.get('stage4',{})); cfg['stage4']['seed_start']=int(st.get('five_label_seed_start',101001)); cfg['stage4']['seeds']=int(st.get('five_label_seeds',8)); cfg['stage4']['trajectory_reps']=reps; cfg['stage4']['labels']=labels
        cfg['stage4']['matched_families']=[{'family':st.get('target_family','target_3d_adaptive_trajectory_lock'),'shapes':[[3,3,2]]}]
        cfg_path=gen/'stage5b0R_five_label_3x3x2.yaml'; cfg_path.write_text(yaml.safe_dump(cfg,sort_keys=False))
        camp=five_dir/'campaign'; diag=five_dir/'diagnosis'
        run([sys.executable,'scripts/run_stage4_publication_campaign.py','--config',str(cfg_path.relative_to(ROOT)),'--out',str(camp.relative_to(ROOT))], transcript, camp/'stage4_publication_campaign_manifest.json')
        run([sys.executable,'scripts/run_stage4a_stability_diagnosis.py','--config',str(cfg_path.relative_to(ROOT)),'--campaign-dir',str(camp.relative_to(ROOT)),'--out',str(diag.relative_to(ROOT))], transcript, diag/'stage4a_stability_manifest.json')
        mech=mechanism_table(camp,target,labels,ci=ci)
        five_ran=True
        core=mech[mech.contrast=='static_only_minus_mobile_plus_spin_density'] if len(mech) else pd.DataFrame()
        comp=mech[mech.contrast.isin(['static_only_minus_mobile_only','static_only_minus_spin_density_only'])] if len(mech) else pd.DataFrame()
        five_passed=bool(len(core) and float(core.mean_fixed_effect_db.iloc[0])<0 and float(core.t_ci_high.iloc[0])<0 and len(comp)>=1)
    gates=pd.DataFrame([
        {'gate':'trajectory_fraction_lock_passed','value':bool(lock_passed)},
        {'gate':'selected_ntraj','value':int(selected['ntraj']) if selected else None},
        {'gate':'selected_trajectory_reps','value':int(selected['trajectory_reps']) if selected else None},
        {'gate':'five_label_mechanism_pilot_ran','value':bool(five_ran)},
        {'gate':'five_label_mechanism_pilot_passed','value':bool(five_passed)},
    ])
    if lock_passed and five_passed:
        route='stage5b0R_passed_prepare_stage5b_full_design_review'
    elif lock_passed:
        route='stage5b0R_lock_passed_five_label_needs_review'
    else:
        route='stage5b0R_trajectory_fraction_lock_needs_more_power'
    gates.loc[len(gates)]={'gate':'route','value':route}
    rec=pd.DataFrame([{'next_stage':'Stage 5B full five-label mechanism decomposition','recommended':bool(lock_passed and five_passed),'target_shape':target,'selected_ntraj':int(selected['ntraj']) if selected else None,'selected_trajectory_reps':int(selected['trajectory_reps']) if selected else None,'recommended_disorder_seeds':max(seeds,16),'labels':'static_only,mobile_only,spin_density_only,mobile_plus_spin_density,everything','decision_basis':'Proceed only after adaptive trajectory-fraction lock and five-label preflight are satisfied.'}])
    save_dataframe(lock/'stage5b0R_adaptive_lock_summary.csv',sdf,raw)
    save_dataframe(lock/'stage5b0R_seed_level_effects.csv',seed_df,raw)
    save_dataframe(lock/'stage5b0R_readiness_gates.csv',gates,raw)
    save_dataframe(five_dir/'stage5b0R_five_label_mechanism_table.csv',mech,raw)
    save_dataframe(out/'stage5b0R_recommended_stage5b_design.csv',rec,raw)
    (lock/'COMMAND_TRANSCRIPT_STAGE5B0R.txt').write_text('\n'.join(transcript)+'\n')
    save_json(lock/'stage5b0R_manifest.json',{'stage':'stage5b0R_adaptive_trajectory_fraction_lock_mechanism_pilot','created_utc':datetime.datetime.now(datetime.UTC).isoformat(),'config':args.config,'parent_config_hash':parent_hash,'target_shape':target,'candidates':candidates,'selected_candidate':selected,'trajectory_fraction_threshold':traj_thr,'lock_passed':bool(lock_passed),'five_label_pilot_ran':bool(five_ran),'five_label_pilot_passed':bool(five_passed),'route':route,'claim_scope':'gated preflight only; no broad mechanism, finite-size, or publication claim'})
    print(f'stage5b0R adaptive trajectory lock/mechanism pilot wrote {out}; route={route}; lock_passed={lock_passed}; five_label_ran={five_ran}')
