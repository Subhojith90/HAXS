...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--results',default='results/stage5a_lite'); ap.add_argument('--figures',default='figures/stage5a_lite'); ap.add_argument('--out',default='manuscript/stage5a_lite'); args=ap.parse_args()
    res=ROOT/args.results; base=res/'convergence_replication'; out=ROOT/args.out; out.mkdir(parents=True,exist_ok=True)
    decision=json.loads((res/'decision/stage5a_decision.json').read_text()) if (res/'decision/stage5a_decision.json').exists() else {}
    ...
    lines=['# Stage 5A Convergence and Replication Gate Report','',f"**Route:** `{decision.get('route','unknown')}`",f"**Passed:** `{decision.get('passed',False)}`",'','## Scope','Stage 5A tests whether the single surviving `3x3x2` mechanism signal is trajectory-converged and independently seed-replicated. It does not make publication claims.','','## Summary']
    ...
    (out/'stage5a_report.md').write_text('\n'.join(lines)+'\n')
    print(f'stage5a report wrote {out}/stage5a_report.md')
