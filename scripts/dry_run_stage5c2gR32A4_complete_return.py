...
from finalize_stage5c2gR32A4_authorization import _safe_extract, verify_complete_g0_return
...
def _run_synthetic_host(
    ...
    completed = subprocess.run(
        [
            sys.executable, "-I", "-B",
            str(execution_root / "scripts/run_stage5c2gR32A4_g0.py"),
...
def main() -> None:
    ...
    forbidden = [ROOT / path.relative_to(ROOT) for path in [
        AUTHORIZATION_PATH, LOCK_PATH, RECEIPT_PATH, STATE_PATH,
    ]]
    ...
    with tempfile.TemporaryDirectory(
        prefix="haxs-stage5c2gR32A4-local-protocol-"
    ) as directory:
    ...
    with tempfile.TemporaryDirectory(
        ...
        completed = subprocess.run(
            [
                sys.executable, "-I", "-B",
                str(packaging_root / "scripts/package_stage5c2gR32A4_supervisor_return.py"),
    ...
    archive = deliverables / "HAXS_Stage5C2G_R3_2A_4_Complete_G0_Return.zip"
    candidate = load_candidate()
    with tempfile.TemporaryDirectory(prefix="haxs-stage5c2gR32A4-local-return-") as directory:
    ...
    decision = {
        "schema_version": "haxs.stage5c2gR32A4.local-two-host-dry-run.v1",
