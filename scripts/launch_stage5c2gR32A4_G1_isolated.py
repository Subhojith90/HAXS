...
from verify_stage5c2gR32A4_environment import verify_environment
...
def load_authorization(candidate: dict) -> dict:
    ...
    if (
        authorization.get("schema_version") != "haxs.stage5c2gR32A4.atomic-authorization.v1"
        ...
        raise RuntimeError("atomic R3.2A.4 authorization failed")
...
def preflight_and_reserve(candidate: dict, authorization: dict, root: Path = ROOT) -> tuple[dict, dict]:
    ...
    setup = {
        "schema_version": "haxs.stage5c2gR32A4.setup-preflight.v1", "status": "PASS",
...
def main() -> None:
    if not sys.flags.isolated or len(sys.argv) != 1:
        raise RuntimeError("official R3.2A.4 G1 requires isolated no-override execution")
    ...
    try:
        completed = subprocess.run(
            [sys.executable, "-I", "-B", "scripts/run_stage5c2gR32A4_G1.py"],
