...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--results',default='results/stage4f_lite'); ap.add_argument('--out',default='results/stage4f_lite/decision')
    args=ap.parse_args(); res=ROOT/args.results; out=ensure_dir(ROOT/args.out)
    man=json.loads((res/'high_trajectory_confirmatory/stage4f_confirmatory_manifest.json').read_text())
    ...
    decision={
        'stage':'stage4f_3x3x2_high_trajectory_confirmatory',
        'route':route,
        ...
        'stage5_design_review_ready': route in ['stage4f_confirmatory_passed_prepare_stage5_design_review','stage4f_signal_survives_needs_stage5_power'],
    ...
    save_json(out/'stage4f_decision.json',decision); save_dataframe(out/'stage4f_decision_table.csv',tab,{})
    print(f'stage4f decision wrote {out}; route={route}')
