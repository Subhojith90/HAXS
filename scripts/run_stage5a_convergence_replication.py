...
def main():
    ...
    ap.add_argument('--config',default='configs/stage5a_lite/convergence_replication_3x3x2.yaml')
    ap.add_argument('--out',default='results/stage5a_lite/convergence_replication')
    args=ap.parse_args()
    ...
    for block, seed_start in blocks:
        for ntraj in ntraj_sweep:
            ...
            run([sys.executable,'scripts/run_stage4_publication_campaign.py','--config',str(cfg_path.relative_to(ROOT)),'--out',str(camp.relative_to(ROOT))],transcript)
            run([sys.executable,'scripts/run_stage4a_stability_diagnosis.py','--config',str(cfg_path.relative_to(ROOT)),'--campaign-dir',str(camp.relative_to(ROOT)),'--out',str(diag.relative_to(ROOT))],transcript)
    ...
    route='stage5a_passed_prepare_stage5b_mechanism_decomposition' if passed else ('stage5a_replication_directional_needs_more_power' if repl['mean_fixed_effect_db']<0 else 'stage5a_replication_failed_reconsider_route')
    gates=pd.DataFrame([
    ...
    save_json(out/'stage5a_manifest.json',{'stage':'stage5a_convergence_replication_gate','config':args.config,'parent_config_hash':parent_hash,'target_shape':target,'ntraj_sweep':ntraj_sweep,'blocks':[b for b,_ in blocks],'seeds_per_block':seeds,'trajectory_reps':reps,'route':route,'passed':passed,'claim_scope':'convergence and replication gate only; no publication claim'})
    print(f'stage5a convergence replication wrote {out}; route={route}; passed={passed}; final_ntraj={final_n}')
