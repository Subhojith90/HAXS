...
from stage5c2gR32A5_semantics import verify_command_record, verify_junit_semantics
...
def verify_host_record(
    ...
    container = evidence_root.resolve(strict=True)
    host_root = path.parent.resolve(strict=True)
    try:
        host_root.relative_to(container)
    except ValueError as error:
        raise RuntimeError(f"{label} evidence root escapes its return container") from error
    if path.is_symlink() or host_root.is_symlink():
        raise RuntimeError(f"{label} evidence root is symlinked")
    ...
    if set(host) != required:
        raise RuntimeError(f"{label} host record keys differ from the R3.2A.5 schema")
    if (
        host["schema_version"] != "haxs.stage5c2gR32A5.physical-host-g0.v1"
        or host["status"] != "PASS"
    ...
    full_path = _bound_file(host_root, primary["full_junit"], f"{label} full JUnit")
    targeted_path = _bound_file(host_root, primary["targeted_junit"], f"{label} targeted JUnit")
    ledger_path = _bound_file(host_root, primary["named_test_ledger"], f"{label} ledger")
    ledger = strict_json(ledger_path)
    if (
        ledger.get("schema_version") != "haxs.stage5c2gR32A5.named-tests.v1"
    ...
    record_paths = {
        name: _bound_file(host_root, record, f"{label} {name} command record")
        for name, record in records.items()
    }
    python_sha = candidate["python_executable_sha256"]
    compile_record = verify_command_record(
        record_paths["compileall"], host_root, "compileall",
        [
            "{BOUND_PYTHON}", "-I", "-B", "-X", "pycache_prefix={PYCACHE}",
            "-m", "compileall", "-q", "src", "scripts", "scripts_patch", "tests",
        ],
        0, python_sha, None,
    )
    full_record = verify_command_record(
        record_paths["full_tests"], host_root, "full_tests",
    ...
    targeted_record = verify_command_record(
        record_paths["targeted_tests"], host_root, "targeted_tests",
...
def recompute_two_host_g0(
    ...
    return {
        "schema_version": "haxs.stage5c2gR32A5.two-host-g0.v1",
        "status": "PASS",
