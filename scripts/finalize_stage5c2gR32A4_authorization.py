...
def verify_complete_g0_return(
    ...
    required = {
        ...
        "protocol_content_sha256", "transport_container_sha256", "protocol_path", "host_a_path",
        "host_b_path", "comparison_path", "files", "scientific_execution_performed",
        "G1_authorized", "synthetic_dry_run", "return_sha256",
    }
    if set(record) != required:
        raise RuntimeError("complete R3.2A.4 G0 return schema failed")
    canonical = {key: value for key, value in record.items() if key != "return_sha256"}
    if (
        record["schema_version"] != "haxs.stage5c2gR32A4.complete-g0-return.v1"
    ...
    if (
        not returned_protocol.is_file()
        or returned_protocol.is_symlink()
...
def validate_receipt(
    ...
    if (
        receipt["schema_version"] != "haxs.stage5c2gR32A4.authorization.v1"
...
def authorize(
    ...
    if not receipt_path.is_file() or receipt_path.is_symlink():
        raise RuntimeError("structured supervisor receipt is missing or symlinked")
    if not protocol.is_file() or protocol.is_symlink() or protocol.suffix.lower() != ".zip":
        raise RuntimeError("official protocol must be one safe ZIP file")
    ...
    if protocol_verifier is None:
        from verify_stage5c2gR32A4_fresh_unzip import verify_protocol
    ...
    with tempfile.TemporaryDirectory(prefix="haxs-stage5c2gR32A4-finalizer-") as directory:
        if (
            not g0_return.is_file()
            or g0_return.is_symlink()
            or g0_return.suffix.lower() != ".zip"
        ):
            raise RuntimeError("official complete G0 return must be one safe ZIP file")
    ...
    payload = {
        "schema_version": "haxs.stage5c2gR32A4.atomic-authorization.v1",
...
def main() -> None:
    ...
    result = authorize(
        ...
        args.g0_return.absolute(), args.dry_run,
    )
