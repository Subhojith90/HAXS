...
def main():
    ...
    ap.add_argument('--config',default='configs/stage5aR_lite/convergence_replication_3x3x2_repaired.yaml')
    ap.add_argument('--out',default='results/stage5aR_lite/convergence_replication')
    args=ap.parse_args()
    ...
    save_json(out/'stage5a_manifest.json',{'stage':'stage5aR_repaired_high_trajectory_convergence_gate','config':args.config,'parent_config_hash':parent_hash,'target_shape':target,'ntraj_sweep':ntraj_sweep,'blocks':[b for b,_ in blocks],'seeds_per_block':seeds,'trajectory_reps':reps,'route':route,'passed':passed,'claim_scope':'repaired high-trajectory convergence and independent seed replication gate only; no publication claim'})
    print(f'stage5aR repaired convergence replication wrote {out}; route={route}; passed={passed}; final_ntraj={final_n}')
