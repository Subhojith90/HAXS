...
def main() -> None:
    ...
    parser.add_argument("--out", type=Path, default=ROOT / "results/stage5c2gR32A3/environment.json")
    args = parser.parse_args()
    if not sys.flags.isolated or sys.version_info[:3] != (3, 12, 7) or platform.python_implementation() != "CPython":
        raise RuntimeError("R3.2A.3 environment writer requires isolated CPython 3.12.7")
    if args.out.exists() or args.out.is_symlink():
        raise RuntimeError("refusing to overwrite R3.2A.3 environment")
    ...
    with tempfile.TemporaryDirectory(prefix="haxs-stage5c2gR32A3-env-") as directory:
        ...
        if completed.returncode:
            raise RuntimeError("immutable R3.2A.3 wheel installation failed")
        installed = installed_wheel_tree_identity(target, wheel)
    payload = {
        "schema_version": "haxs.stage5c2gR32A3.environment.v1",
        **current_environment(), "haxs_version": "0.8.6",
