...
def verify_root(root: Path, candidate: dict | None = None) -> dict:
    ...
    if (
        manifest.get("schema_version") != "haxs.stage5c2gR32A4.root-manifest.v1"
        ...
        raise RuntimeError("candidate-bound R3.2A.4 root manifest failed")
    ...
    return {
        "stage": "stage5c2gR32A4_root", "status": "PASS",
