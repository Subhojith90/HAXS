...
def main():
    ap=argparse.ArgumentParser(); ap.add_argument('--results',default='results/stage5aR_lite'); ap.add_argument('--out',default='results/stage5aR_lite/decision'); args=ap.parse_args()
    ...
    pd.DataFrame(table).to_csv(out/'stage5aR_decision_table.csv',index=False)
    (out/'stage5aR_decision.json').write_text(json.dumps({'stage':'stage5aR_repaired_high_trajectory_convergence_gate','route':route,'passed':passed,'publication_claim_allowed':False,'stage5b_design_review_allowed':bool(passed),'note':'Stage 5A-R repairs the Stage 5A estimator with higher-trajectory convergence and independent replication gates; no manuscript claim.'},indent=2))
