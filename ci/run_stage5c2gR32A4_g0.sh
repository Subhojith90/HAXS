...
RELEASE="$REPOSITORY_ROOT/releases/stage5c2gR32A4"
...
PROTOCOL="${HAXS_PROTOCOL_ARCHIVE:-$RELEASE/HAXS_Stage5C2G_R3_2A_4_Protocol.zip}"
SIDECAR="${HAXS_PROTOCOL_SIDECAR:-$RELEASE/HAXS_Stage5C2G_R3_2A_4_Protocol_SHA256.txt}"
PROTOCOL_PARENT="$RUN/protocol"
EXTRACTED="$PROTOCOL_PARENT/HAXS_Stage5C2G_R3_2A_4_Protocol"
...
if [[ ! -f "$PROTOCOL" ]]; then
  CHUNK_MANIFEST="$RELEASE/HAXS_Stage5C2G_R3_2A_4_Protocol_CHUNKS_SHA256.txt"
  CHUNK_DIR="$RELEASE/HAXS_Stage5C2G_R3_2A_4_Protocol.chunks"
  [[ -f "$CHUNK_MANIFEST" && -d "$CHUNK_DIR" ]]
  (cd "$RELEASE" && shasum -a 256 -c "$(basename "$CHUNK_MANIFEST")")
  shopt -s nullglob
  CHUNKS=("$CHUNK_DIR"/HAXS_Stage5C2G_R3_2A_4_Protocol.zip.part-*)
  shopt -u nullglob
  [[ "${#CHUNKS[@]}" -ge 2 ]]
  PROTOCOL="$RUN/transport/HAXS_Stage5C2G_R3_2A_4_Protocol.zip"
  cat "${CHUNKS[@]}" > "$PROTOCOL"
fi
...
"$PY" -I -m pip install --no-index --no-deps \
  "$EXTRACTED/output/stage5c2gR32A4/haxs-0.8.7-py3-none-any.whl"

cd "$EXTRACTED"
HAXS_R32A4_PYTHON="$PY" HAXS_RUN_ROOT="$RUN/g0" \
  HAXS_PROTOCOL_ARCHIVE="$PROTOCOL" HAXS_PROTOCOL_SIDECAR="$SIDECAR" \
  bash run_stage5c2gR32A4_G0.sh --host-label "$HOST_LABEL"
