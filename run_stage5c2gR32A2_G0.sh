...
: "${HAXS_R32A2_PYTHON:?HAXS_R32A2_PYTHON must be the exact isolated CPython 3.12.7 environment}"

REPOSITORY_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROTOCOL="${HAXS_PROTOCOL_ARCHIVE:-$REPOSITORY_ROOT/output/stage5c2gR32A2/HAXS_Stage5C2G_R3_2A_2_Protocol.zip}"
SIDECAR="${HAXS_PROTOCOL_SIDECAR:-$REPOSITORY_ROOT/output/stage5c2gR32A2/HAXS_Stage5C2G_R3_2A_2_Protocol_SHA256.txt}"
RUN="$HAXS_RUN_ROOT"
ROOT="$RUN/protocol/HAXS_Stage5C2G_R3_2A_2_Protocol"
...
# The protocol tree is an immutable input.  All transcripts, JUnit, caches and
# scientific artifacts belong under the external run root, never beside the
# candidate-bound files.
chmod -R a-w "$ROOT"
...
cd "$ROOT"
"$HAXS_R32A2_PYTHON" -I -B scripts/run_stage5c2gR32A2_g0.py \
